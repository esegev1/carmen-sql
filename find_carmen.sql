...
SELECT a.name, a.population, b.language FROM country AS a LEFT JOIN countrylanguage AS b ON b.countrycode = a.code WHERE a.region = 'Southern Europe' ORDER BY a.population ASC LIMIT 1; 
-- Italian
...
SELECT 
    a.countrycode, 
    c.name,
    COUNT(DISTINCT b.language) 
FROM countrylanguage AS a 
LEFT JOIN countrylanguage AS b ON a.countrycode = b.countrycode 
LEFT JOIN country AS c ON a.countrycode = c.code
...
WHERE TRUE
    AND 
    AND a.population > 91000
    AND a.population < 92000
    AND a.population % 100 = 84;
